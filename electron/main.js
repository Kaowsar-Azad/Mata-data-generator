...
ipcMain.handle('generate-eps-jpg', async (event, inputPath, addWhiteBgToPng = true) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      const gsEnv2 = { ...process.env, GS_LIB: gsLibPath2, PATH: `${path.join(bundledBinDir2, 'win_graphics_proc', 'bin')};${process.env.PATH || ''}` };
...
ipcMain.handle('remove-bg-local', async (event, inputPath) => {
  ...
  try {
    ...
    console.log('[IPC remove-bg-local] Spawning standalone background removal process in Electron...');
