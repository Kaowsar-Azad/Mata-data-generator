async function scrapeTextSearch({ query, order, contentType, gentech, page = 1 }, BrowserWindow, fileLog) {
  return new Promise((resolve) => {
    fileLog(`[Dreamstime Scraper] Starting search. Query: "${query}", Page: ${page}`);
    ...
    let url = `https://www.dreamstime.com/search.php?srh_field=${encodeURIComponent(query)}&pg=${page}`;
    
    fileLog(`[Dreamstime Scraper] Loading URL: ${url}`);
          ...
          const debugInfo = await scraperWindow.webContents.executeJavaScript(`
            (async () => {
              ...
              document.querySelectorAll('img').forEach(img => {
                ...
                if (src && src.includes('dreamstime.com')) {
                  const link = img.closest('a');
                  const detailUrl = link ? link.href : '';
                  uniqueImages.set(src, { src, alt: img.alt || 'Dreamstime Image', detailUrl });
                }
              });
    ...
    setTimeout(() => {
      if (scraperWindow && !scraperWindow.isDestroyed()) {
        scraperWindow.destroy();
        resolve({ success: false, error: 'Timeout waiting for Dreamstime to load.' });
      }
    }, 20000);
  });
}
