async function scrapeTextSearch({ query, order, contentType, gentech, page = 1 }, BrowserWindow, fileLog) {
  return new Promise((resolve) => {
    fileLog(`[Freepik Scraper] Starting search. Query: "${query}", Page: ${page}`);
    ...
    let url = `https://www.freepik.com/search?format=search&query=${encodeURIComponent(query)}&page=${page}`;
