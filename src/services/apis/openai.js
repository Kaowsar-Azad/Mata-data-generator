...
/**
 * Build the metadata prompt depending on file context for OpenAI.
 */
export function buildOpenAIPrompt({ isEps, isPlaceholder, isVideo, fileName, extractedTextContext, promptSettings, skipPolicyScan }) {
  ...
  const promptKeywordsCount = Math.min(100, s.keywordCount + 25);
...
Generate EXACTLY ${typeof s !== 'undefined' && s.smartMode ? '15 to 30' : promptKeywordsCount} keywords total. There is only ONE ranking system in this prompt: the tier order below. Output the keyword list in this exact tier sequence — Tier 1 keywords first, Tier 6 keywords last. Because Adobe Stock and Shutterstock weight earlier keyword positions more heavily in search, this tier order IS the ranking. Do not apply any separate position/slot scheme on top of it — there isn't one.

Allocate the ${typeof s !== 'undefined' && s.smartMode ? '15 to 30' : promptKeywordsCount} keywords across tiers using these target proportions. Round each to the nearest whole number, then adjust Tier 6 up or down so the total matches ${typeof s !== 'undefined' && s.smartMode ? '15 to 30' : promptKeywordsCount} exactly:
...
- TIER 6 — HIGH-VALUE SYNONYMS (remainder — fill exactly to reach ${typeof s !== 'undefined' && s.smartMode ? '15 to 30' : promptKeywordsCount}): closely related, highly searched commercial terms not already used elsewhere in the list.
