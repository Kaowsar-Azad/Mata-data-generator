...
import { GeneratePromptsConfig } from './generator';
 ...
 */
export async function generateAIPrompts({ config, apiKeys, apiProvider }: AIGeneratorArgs) {
  let {
    categoryName = 'auto',
    mainCategory = 'auto',
    mediaType = 'photo',
  ...
  // Pre-filter and shuffle categories for maximum diversity without repetition
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (!currentMain || currentMain === 'auto') {
      currentMain = shuffledMainKeys[i % shuffledMainKeys.length];
    }

    if (!currentSub || currentSub === 'auto') {
      const subs = mainCategories[currentMain] || [];
      if (subs.length > 0) {
        // Pre-shuffle subcategories to avoid selecting the same subcategory if the same main is picked again
        const shuffledSubs = [...subs].sort(() => 0.5 - Math.random());
        currentSub = shuffledSubs[i % shuffledSubs.length];
      } else {
        // fallback
    ...
    if (resolvedCategory === '3D Icons' || currentMain === 'Icons') {
      ...
      if (iconLayout === 'single') {
        aiSpecificSubject = `a standard UI/UX core concept or web icon subject (e.g., security shield, analytics chart, user profile, settings gear, cloud storage, etc.), but designed with a highly premium, modern, and eye-catching aesthetic (material: ${prefix || 'any premium material'}, style: ${iconStyle === 'monochrome' ? 'monochrome line art' : 'colorful'})`;
      } else {
        const themes = [
...
4. Detail & Specificity: Provide extremely vivid and descriptive details for photos and 3D renders. HOWEVER, for flat vector graphics or icons, you MUST enforce strict minimalism, clean geometry, and scalable simplicity so they can be easily auto-traced.
...
7. ICONS MINIMALISM (CRITICAL): If generating Icons, you MUST strictly enforce a clean, minimalist, flat design aesthetic (unless it's a 3D icon). Do NOT add extreme details, textures, complex backgrounds, or convoluted scenes. The icons must look like simple, professional UI/UX assets or scalable vector logos (e.g., Apple iOS icons, Google Material Design, or clean glyphs). They must be highly suitable for auto-tracing into vector formats. Avoid overly complex nodes, gears, or intricate patterns.
8. NO MARKDOWN: Output absolutely NO markdown formatting (do NOT use **bold**, *italics*, or bullet points). The generated prompts must be plain text only.
