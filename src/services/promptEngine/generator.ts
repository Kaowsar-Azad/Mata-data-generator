...
const generateSinglePrompt = (categoryName, mediaType, promptLength, styleChoice, lightingChoice, cameraChoice, customInstruction, mainCategory, targetModel = 'default', aspectRatio = '16:9', promptIndex = 0, iconLayout = 'set', iconStyle = 'colorful') => {
  ...
  const pStyle = styleChoice === 'auto' ? (dynamicVocabulary?.Style?.length > 0 ? getRandom(dynamicVocabulary.Style) : getRandom(styles)) : styleChoice;
  const pLighting = lightingChoice === 'auto' ? (dynamicVocabulary?.Lighting?.length > 0 ? getRandom(dynamicVocabulary.Lighting) : getRandom(lighting)) : lightingChoice;
  const pCamera = cameraChoice === 'auto' ? (dynamicVocabulary?.Camera?.length > 0 ? getRandom(dynamicVocabulary.Camera) : getRandom(cameraAngles)) : cameraChoice;
  
  // Get category specific elements
  // We prioritize the highly optimized, curated templates in categoryData to ensure stock market quality
  let subject = getRandom(categoryData.subjects);
  let environment = getRandom(categoryData.environments);
  let action = getRandom(categoryData.actions);
  
  // Optionally append a dynamic concept if one exists, but keep it subtle
  const dynamicConcept = dynamicVocabulary?.Concept?.length > 0 && Math.random() > 0.7 ? getRandom(dynamicVocabulary.Concept) : null;
  if (dynamicConcept) {
    action = `${action} (theme: ${dynamicConcept})`;
  }
  ...
  if (mainCategory === 'Icons' || resolvedCategory === '3D Icons') {
    ...
    if (iconLayout === 'single') {
      ...
      const baseIndex = Math.floor(promptIndex) % baseIcons.length;
      ...
      if (resolvedCategory === '3D Icons') {
        ...
        const subjectStructuresSingle3D = [
            `single 3D ${prefix ? prefix + ' ' : ''}${baseIcon} icon`,
            `highly detailed 3D render of a ${prefix ? prefix + ' ' : ''}${baseIcon} icon`,
            `a modern 3D ${prefix ? prefix + ' ' : ''}${baseIcon} symbol`,
            `creative 3D illustration of a ${prefix ? prefix + ' ' : ''}${baseIcon} UI element`
        ];
        
        subject = subjectStructuresSingle3D[Math.floor(promptIndex) % subjectStructuresSingle3D.length];
      } else {
        let cleanName = resolvedCategory.toLowerCase().replace(' icons', '');
        const genericNames = ['flat', 'line', 'glyph', 'vector', 'ui elements', 'outline', 'filled', 'random icon layouts, grid, and set', 'auto'];
        
        if (categoryName === 'auto' || genericNames.includes(cleanName) || mainCategory === 'Icons' && categoryName === 'auto') {
            cleanName = baseIcon;
        } else {
            const specificSubjects = categoryData?.subjects || [];
            if (specificSubjects.length > 0) {
                cleanName = getRandom(specificSubjects);
                // Remove 'a ' or 'an ' if present so it flows better
                cleanName = cleanName.replace(/^(a|an)\s+/i, '');
            }
        }
        
        let subjectStructuresSingleFlat = [
            `premium highly-detailed isometric vector icon of a ${cleanName}`,
            `modern vibrant gradient app logo UI element depicting a ${cleanName}`,
            `trendy glossy glassmorphism interface icon of a ${cleanName}`,
            `high-end minimalist duotone vector symbol of a ${cleanName}`,
            `professional sleek UI icon design representing a ${cleanName}`,
            `creative, colorful, and highly polished app icon of a ${cleanName}`
        ];
        
        subject = subjectStructuresSingleFlat[Math.floor(promptIndex) % subjectStructuresSingleFlat.length];
      }
    } else {
      ...
      const gridSizes = [
        'set of 4', 'set of 6', 'set of 9', 'set of 12', 'set of 16',
        '2x2 grid of', '3x2 grid of', '3x3 grid of', '4x3 grid of', '4x4 grid of',
        'collection of 5', 'collection of 8', 'pack of 10'
      ];
      
      const gridIndex = Math.floor(promptIndex) % gridSizes.length;
      const grid = gridSizes[gridIndex];
      
      if (resolvedCategory === '3D Icons') {
        const themeIndex = Math.floor(promptIndex / gridSizes.length) % themes.length;
        ...
        const color = colors[colorIndex];
        const prefix = [material, color].filter(Boolean).join(' ');
        const baseThemeText = prefix ? `3D ${prefix} ${theme}` : `3D ${theme}`;
        
        const subjectStructures3D = [
            `${grid} matching ${baseThemeText} UI icons`,
            `A comprehensive 3D set of ${baseThemeText} icons arranged in a ${grid}`,
            `${grid} of highly detailed ${baseThemeText} symbols`,
            `A diverse ${grid} layout of 3D UI icons focused on ${baseThemeText}`,
            `Collection of ${baseThemeText} themed 3D icons designed as a ${grid}`
        ];
        
        subject = subjectStructures3D[Math.floor(promptIndex) % subjectStructures3D.length];
      } else {
        let cleanName = resolvedCategory.toLowerCase().replace(' icons', '');
        
        const genericNames = ['flat', 'line', 'glyph', 'vector', 'ui elements', 'outline', 'filled', 'random icon layouts, grid, and set', 'auto'];
        if (categoryName === 'auto' && dynamicVocabulary?.Subject?.length > 0) {
            cleanName = getRandom(dynamicVocabulary.Subject);
        } else if (genericNames.includes(cleanName)) {
            const themeIndex = Math.floor(promptIndex) % themes.length;
            cleanName = themes[themeIndex];
        }
        
        const specificDetail = dynamicVocabulary?.Subject?.length > 0 ? getRandom(dynamicVocabulary.Subject) : '';
        
        const subjectStructures = [
            `${grid} matching flat UI icons depicting ${cleanName}`,
            `A comprehensive vector set of ${cleanName} icons arranged in a ${grid}`,
            `${grid} of minimalist ${cleanName} interface symbols`,
            `A diverse ${grid} layout of UI icons focused on ${cleanName}`,
            `Collection of ${cleanName} themed flat icons designed as a ${grid}`,
            `${grid} featuring modern flat UI icons for ${cleanName} applications`,
            `A cohesive ${grid} of ${cleanName} vector elements`,
            `${grid} layout showcasing various ${cleanName} icons`
        ];
        
        const baseSubject = subjectStructures[Math.floor(promptIndex) % subjectStructures.length];
        
        if (specificDetail && Math.random() > 0.3) {
            subject = `${baseSubject}, featuring elements like ${specificDetail}`;
        } else {
            subject = baseSubject;
        }
      }
  ...
  if (resolvedCategory === '3D Icons' || (mainCategory === 'Icons' && mediaType === 'vector')) {
    environment = 'isolated on a pure white seamless background without any transparency';
  }
  ...
  if (targetModel === 'midjourney') {
    ...
    if (mediaType === 'isolated_white') {
      const openings = [
        ...
        `Clean ${pStyle} design of ${subject}, ${action}, isolated on pure white seamless background`,
        `Isolated on pure white seamless background: ${pStyle} ${subject}, ${action}`,
        `Professional ${subject}, ${action}, ${pStyle} design, isolated on white`,
      ];
      rawPrompt = `${selectOpening(openings, promptIndex)}, ${keywords} --v 6.0 --ar ${aspectRatio}`;
    } else {
      const mjOpenings = [
        `${pStyle} ${mediaType} featuring ${subject}, ${action}, ${environment}`,
        `${subject}, ${action}, ${pStyle} ${mediaType}, ${environment}`,
        `Stunning ${pStyle} ${mediaType} of ${subject}, ${action}, ${environment}`,
        `Cinematic ${pStyle} ${mediaType} capturing ${subject} ${action}, ${environment}`,
      ];
    ...
    const openings = [
      `High-quality ${pStyle} ${typeText} of ${subject} ${action}, ${environment}.`,
      `${subject} ${action}, ${environment} — beautifully crafted ${pStyle} ${typeText}.`,
      `Set in ${environment}, a ${pStyle} ${typeText} of ${subject} ${action}.`,
      `Detailed ${typeText} capturing ${subject} ${action}, ${environment}, in a ${pStyle} aesthetic.`,
    ];
    ...
    const openings = [
      `Professional ${pStyle} ${typeText} of ${subject} ${action}, ${environment}.`,
      `${subject} ${action}, ${environment} — stunning ${pStyle} ${typeText}.`,
      `Creative ${pStyle} ${typeText} of ${subject} ${action}, ${environment}.`,
      `Beautiful ${typeText} capturing ${subject} ${action}, ${environment}, in a ${pStyle} style.`,
    ];
    rawPrompt = `${selectOpening(openings, promptIndex)} ${tail}`;
  }
    ...
    const openings = [
      `Vibrant ${pStyle} ${typeText} showcasing ${subject} ${action}, ${environment}.`,
      `Creative ${pStyle} ${typeText} of ${subject} ${action}, ${environment}.`,
      `Outstanding ${pStyle} ${typeText} featuring ${subject} ${action}, ${environment}.`,
      `Stunning ${typeText} featuring ${subject} ${action}, ${environment}, in a ${pStyle} style.`,
    ];
    rawPrompt = `${selectOpening(openings, promptIndex)} ${tail}`;
  }
    ...
    const openings = [
      `Vibrant ${pStyle} ${typeText} of ${subject} ${action}, ${environment},`,
      `${subject} ${action}, ${environment} — ${pStyle} ${typeText},`,
      `Detailed ${pStyle} ${typeText} of ${subject} ${action}, ${environment},`,
      `Striking ${typeText} showing ${subject} ${action}, ${environment}, in a ${pStyle} aesthetic,`,
    ];
    rawPrompt = `${selectOpening(openings, promptIndex)} ${tail}`;
  }
 ...
 */
export interface GeneratePromptsConfig {
  categoryName?: string;
  mainCategory?: string;
  mediaType?: string;
  promptLength?: string;
...
export const generatePrompts = ({
  ...
  // Pre-filter and shuffle categories for maximum diversity
  let availableMainKeys = Object.keys(mainCategories);
  if (mediaType === 'photo' || mediaType === 'video') {
    availableMainKeys = availableMainKeys.filter(k => k !== '3D' && k !== 'Icons' && k !== 'AI Concepts');
  } else if (mediaType === 'vector' || mediaType === 'illustration') {
    availableMainKeys = availableMainKeys.filter(k => k !== '3D');
  } else if (mediaType === '3d') {
    availableMainKeys = availableMainKeys.filter(k => k !== 'Icons');
  }
  const shuffledMainKeys = [...availableMainKeys].sort(() => 0.5 - Math.random());
  ...
  while (uniquePrompts.size < count && attempts < maxAttempts) {
    ...
    let batchMain = mainCategory;
    if (!batchMain || batchMain === 'auto') {
      batchMain = shuffledMainKeys[currentIndex % shuffledMainKeys.length];
    }
    
    let batchSub = categoryName;
    if (!batchSub || batchSub === 'auto') {
      const subs = mainCategories[batchMain] || [];
      if (subs.length > 0) {
        const shuffledSubs = [...subs].sort(() => 0.5 - Math.random());
        batchSub = shuffledSubs[currentIndex % shuffledSubs.length];
      } else {
        batchSub = "Miscellaneous";
      }
    }
